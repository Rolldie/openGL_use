...
namespace OpenGLFunctionOnGraph
{
    ...
    {
        public void gluScreenToWorld(double[] sx, ref double[] wx, double[] sy, ref double[] wy)
        {
            ...
            double worldZ = 0; //variables to hold world x,y,z coordinates
            for (int i = 0; i < sx.Length; i++)
            {
                gluUnProject(sx[i], sy[i], 0, modelview, projection, viewport, ref wx[i], ref wy[i], ref worldZ);
            }
        }
        ...
        private static extern IntPtr _gluErrorString(uint errCode);
        public static string gluErrorString(uint name)
        {
            IntPtr ca = _gluErrorString(name);
            string txt = IntPtrToString(ca);
            return txt;
        }

        [DllImport(GLU_DLL, EntryPoint = "gluGetString")]
        private static extern IntPtr _gluGetString(uint name);
        public static string gluGetString(uint name)
        {
            IntPtr ca = _gluGetString(name);
            string txt = IntPtrToString(ca);
            return txt;
        }
